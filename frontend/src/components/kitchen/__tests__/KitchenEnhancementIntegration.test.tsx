  ...
  useTranslation: () => ({
    t: (key: string, params?: Record<string, unknown>) => {
      const translations: Record<string, string> = {
        ...
        "kitchen.soundsDescription":
          "Play sounds for new orders and status updates",
        ...
        "kitchen.orderReadySoundDesc":
          "Plays when an order is ready for pickup",
        "kitchen.takeawayReadySound": "Takeaway Ready",
        "kitchen.takeawayReadySoundDesc":
          "Plays when a takeaway order is ready",
...
describe("Kitchen Enhancement Integration", () => {
  ...
  describe("Enhanced Kitchen Order Card", () => {
    ...
    it("should handle item checkbox interactions", async () => {
      ...
      await waitFor(() => {
        expect(onItemStatusUpdate).toHaveBeenCalledWith(
          "order-1",
          "1",
          "ready",
        );
  ...
  describe("Sound Settings Component", () => {
    ...
    it("should handle sound testing", async () => {
      ...
      if (newOrderButton && !newOrderButton.hasAttribute("disabled")) {
        ...
        await waitFor(() => {
          expect(kitchenSoundService.testSound).toHaveBeenCalledWith(
            "new_order",
          );
        });
      } else {
  ...
  describe("Business Logic Integration", () => {
    ...
    it("should handle sound notifications correctly", async () => {
      ...
      expect(kitchenSoundService.playOrderReadySound).toHaveBeenCalledWith(
        "order-1",
        "dine_in",
      );
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle sound service errors gracefully", async () => {
      ...
      vi.mocked(kitchenSoundService.testSound).mockRejectedValueOnce(
