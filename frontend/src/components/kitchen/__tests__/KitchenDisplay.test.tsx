...
describe('KitchenDisplay (EnhancedKitchenLayout)', () => {
  ...
  describe('T227: KitchenDisplay_MarksItemsReady', () => {
    it('should call updateOrderStatus API when status changes', async () => {
      ...
      await apiClient.updateOrderStatus('order-1', 'preparing' as any);

      expect(apiClient.updateOrderStatus).toHaveBeenCalledWith('order-1', 'preparing');
    });
    ...
    it('should call updateOrderItemStatus API when item status changes', async () => {
      ...
      expect(apiClient.updateOrderItemStatus).toHaveBeenCalledWith('order-1', 'item-1', 'ready');
    });
  ...
  describe('T229: KitchenDisplay_SoundsNotification', () => {
    ...
    it('should update sound settings', () => {
      ...
      expect(kitchenSoundService.updateSettings).toHaveBeenCalledWith({ enabled: false });
    });

    it('should play sound for new orders', async () => {
      await kitchenSoundService.playNewOrderSound('new-order-id');

      expect(kitchenSoundService.playNewOrderSound).toHaveBeenCalledWith('new-order-id');
    });
  ...
  describe('Additional Kitchen Display Tests', () => {
    ...
    it('should calculate maximum preparation time for an order', () => {
      const order = createMockOrder({
        items: [
          createMockOrderItem({ product: { ...createMockOrderItem().product!, preparation_time: 10 } }),
          createMockOrderItem({ product: { ...createMockOrderItem().product!, preparation_time: 25 } }),
          createMockOrderItem({ product: { ...createMockOrderItem().product!, preparation_time: 15 } }),
        ],
      });
