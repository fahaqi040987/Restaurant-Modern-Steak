...
import {
    Clock, ChefHat, CheckCircle, Bell, Star, ArrowLeft,
    AlertCircle, Loader2, UtensilsCrossed, PartyPopper
} from 'lucide-react'
...
import type { Order, OrderStatus as OrderStatusType, CreateSurveyRequest } from '@/types'
...
const statusOrder: OrderStatusType[] = ['pending', 'confirmed', 'preparing', 'ready', 'served', 'completed']
...
function OrderStatusPage() {
    ...
    const { data: orderData, isLoading, error, refetch } = useQuery({
        ...
        queryFn: async () => {
            const response = await apiClient.get<{ success: boolean; data: Order }>(`/customer/orders/${orderId}`)
    ...
    useEffect(() => {
        if (order && order.status !== previousStatus) {
            ...
            if ((order.status === 'served' || order.status === 'completed') && !surveySubmitted) {
    ...
    const surveyMutation = useMutation({
        mutationFn: (data: CreateSurveyRequest) => apiClient.createSurvey(orderId, data),
        onSuccess: () => {
    ...
    return (
        <div className="min-h-screen bg-[var(--public-bg-primary)] pb-8">
            ...
            <div className="container mx-auto px-4 py-6 max-w-lg">
                {/* Current Status Card */}
                <Card className={`mb-6 border-2 ${currentStatus.color.replace('text-', 'border-')}`}>
                    <CardContent className="pt-6 text-center">
                        <div className={`w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center ${currentStatus.bgColor}`}>
                ...
                <Card className="mb-6 bg-[var(--public-bg-elevated)] border-[var(--public-border)]">
                    ...
                    <CardContent>
                        ...
                        <div className="border-t border-[var(--public-border)] pt-4 space-y-2">
                            ...
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--public-text-secondary)]">Pajak (11%)</span>
                                <span className="text-[var(--public-text-primary)]">
                ...
                {(order.status === 'served' || order.status === 'completed') && !surveySubmitted && (
